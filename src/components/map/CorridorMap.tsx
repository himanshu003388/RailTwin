...
export const CorridorMap: React.FC = () => {
  ...
  return (
    <div className="relative w-full h-full">
      ...
      {/* ═══ Map Legend — Station Risk / Weather / Trains ═══ */}
      {/* Desktop inline — always in DOM, hidden on mobile via CSS */}
      ...
      {showMapLegend && (
        <div className="fixed inset-0 z-50 flex flex-col sm:hidden" style={{ backgroundColor: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(4px)' }} onClick={() => setShowMapLegend(false)}>
          <div className="mt-auto bg-bg-elevated/95 backdrop-blur-md border-t border-border-default rounded-t-2xl shadow-2xl p-4 animate-slide-up max-h-[70dvh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            <div className="mb-3">
              <div className="text-[9px] font-bold text-text-tertiary uppercase tracking-[0.12em] mb-2">Weather</div>
              <div className="flex flex-col gap-1.5">
                <div className="flex items-center gap-2"><span className="w-3 h-3 rounded-full flex-shrink-0 flex items-center justify-center text-[8px] bg-blue-500/10 border border-blue-500/30 text-blue-500">🌧</span><span className="text-[10px] font-medium text-text-secondary">Rainfall</span></div>
                <div className="flex items-center gap-2"><span className="w-3 h-3 rounded-full flex-shrink-0 flex items-center justify-center text-[8px] bg-slate-500/10 border border-slate-500/30 text-slate-400">🌫</span><span className="text-[10px] font-medium text-text-secondary">Low Vis</span></div>
              </div>
            </div>
            <div className="h-px bg-border-default my-2.5" />
            <div className="mb-3">
              ...
              <div className="flex flex-col gap-1.5">
                {[
                  { id: '12301', name: 'Howrah Raj', color: '#3b82f6' }, { id: '12302', name: 'NDLS Raj', color: '#06b6d4' }, { id: '12305', name: 'Patna Raj', color: '#22c55e' }, { id: '12306', name: 'NDLS Raj', color: '#10b981' }, { id: '12259', name: 'Duronto', color: '#ef4444' }, { id: '12381', name: 'Poorva', color: '#a855f7' }, { id: '12382', name: 'Poorva', color: '#ec4899' },
                ].map(t => (
                  <div key={t.id} className="flex items-center gap-2">
                    <span className="w-4 h-[3px] rounded-full flex-shrink-0" style={{ background: t.color, boxShadow: `0 0 6px ${t.color}44` }} />
                    <span className="text-[10px] font-medium text-text-secondary flex-1 truncate">{t.name}</span>
      ...
      {/* Collapsed Live Weather button — mobile only */}
      <button
        onClick={() => setShowLiveWeather(true)}
        className="sm:hidden absolute bottom-3 left-3 z-30 flex items-center gap-1.5 px-2.5 py-2 rounded-xl shadow-lg border border-border-default transition-all duration-150 active:scale-95 cursor-pointer"
        style={{
          background: 'var(--color-bg-elevated)',
          color: 'var(--color-text-secondary)',
        }}
      ...
      >
        <svg width="14" height="14" viewBox="0 0 12 12" fill="none" className="text-accent-blue">
      ...
      {showLiveWeather && (
        <div className="fixed inset-0 z-50 flex flex-col sm:hidden" style={{ backgroundColor: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(4px)' }} onClick={() => setShowLiveWeather(false)}>
          <div className="mt-auto bg-bg-elevated/95 backdrop-blur-md border-t border-border-default rounded-t-2xl shadow-2xl p-4 animate-slide-up max-h-[70dvh] flex flex-col" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between border-b border-border-subtle pb-2 mb-3">
              ...
              <button onClick={() => setShowLiveWeather(false)} className="text-text-tertiary hover:text-text-primary cursor-pointer">
                <svg width="16" height="16" viewBox="0 0 12 12" fill="none">
                  <path d="M3 3L9 9M9 3L3 9" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/>
                </svg>
              </button>
